    ...
    if (!accountId) {
      ...
      try {
        ...
        if (stripeError && typeof stripeError === 'object') {
          ...
          console.error('Error message:', (stripeError as any).message);
          console.error('Error param:', (stripeError as any).param);
          console.error('Full error:', JSON.stringify(stripeError, null, 2));
          
          // Return specific Stripe error for debugging
          throw new Error(`Stripe Connect Error: ${(stripeError as any).code} - ${(stripeError as any).message}`);
        }
        throw new Error('Stripe Connect Error: Unknown error occurred');
      }
    } else {
