...
serve(async (req) => {
  ...
  try {
    ...
    // Check if affiliate has a USD wallet, create if not exists
    ...
    if (!wallet) {
      logStep("Creating USD wallet for affiliate");
    ...
    // Update wallet balance using atomic operation
    const { error: walletError } = await supabaseService.rpc('update_wallet_balance_atomic', {
      wallet_id: wallet.id,
      amount_change: commissionAmount
    });
    ...
    const { error: transactionError } = await supabaseService
      ...
      .insert({
        ...
        type: 'credit',
        status: 'completed',
        description: `Affiliate commission for referral ${sessionId}`,
        object_type: 'deposit'
      });

    if (transactionError) {
      logStep("Warning: Failed to create transaction record", { error: transactionError });
    }
