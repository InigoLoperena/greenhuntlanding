...
serve(async (req) => {
  try {
    ...
    if (!signature) {
      await supabaseClient.rpc('log_security_audit', {
        p_event_type: 'stripe_webhook_no_signature',
        p_details: { headers: Object.fromEntries(req.headers.entries()) },
        p_severity: 'high'
      });
    ...
    if (!webhookSecret) {
      ...
      await supabaseClient.rpc('log_security_audit', {
        p_event_type: 'stripe_webhook_secret_missing',
        p_details: {},
        p_severity: 'critical'
      });
    ...
    try {
      ...
      await supabaseClient.rpc('log_security_audit', {
        p_event_type: 'stripe_webhook_invalid_signature',
        p_details: { error: errMessage },
        p_severity: 'high'
      });
    ...
    await supabaseClient.rpc('log_security_audit', {
      p_event_type: 'stripe_webhook_verified',
      p_details: { 
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (isCoordinatePurchase) {
        ...
        const platformFee = Math.round(amount * 0.2 * 100) / 100; // 20% platform fee with 2 decimals
        const sellerAmount = Math.round((amount - platformFee) * 100) / 100; // Seller gets remainder with 2 decimals
