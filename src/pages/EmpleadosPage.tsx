    ...
    <Card className="bg-zinc-900 border-zinc-800">
      ...
      <CardContent className="space-y-4">
        ...
        {(() => {
          ...
          if (months.length === 0) {
            return (
              <div className="p-4 rounded-lg bg-zinc-950 border-2 border-[#a2c041]/40 text-white font-permanent-marker text-lg">
                Sin registros de horarios aún.
              </div>
            );
          }
          return (
            <div className="space-y-3">
              {months.map(({ key, label, entries: monthEntries }) => (
                <Collapsible key={key} defaultOpen={key === currentMonthKey}>
                  <CollapsibleTrigger className="w-full flex items-center justify-between gap-2 p-3 rounded-lg bg-zinc-800 border-2 border-[#a2c041]/40 hover:border-[#a2c041] transition-colors group">
                    <div className="flex items-center gap-2 text-white font-permanent-marker text-lg">
                      <CalendarClock className="w-5 h-5 text-[#b4fa74]" />
                      Registro de horarios — {label} ({monthEntries.length})
                    </div>
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="border-zinc-800">
                            ...
                            <TableHead className="text-white w-32">Acciones</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
          ...
          <TabsList className="w-full flex flex-wrap h-auto gap-1 bg-zinc-900 border border-zinc-800 p-1">
            {EMPLOYEES.map((name) => (
              <TabsTrigger
                key={name}
                value={name}
                className="font-permanent-marker text-base flex-1 min-w-[100px] data-[state=active]:bg-[#a2c041] data-[state=active]:text-[#611a5a] data-[state=inactive]:bg-zinc-800 data-[state=inactive]:!text-[#b4fa74] data-[state=inactive]:shadow-[0_0_10px_rgba(180,250,116,0.45)] data-[state=inactive]:hover:bg-zinc-700"
              >
                {name}
              </TabsTrigger>
            ))}
